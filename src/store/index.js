...
export default new Vuex.Store({
  ...
  mutations: {
    ADD_ALBUM_POP: (state, album)=>{
      let lastID = parseInt(state.pop[state.pop.length-1].id.match(/\d+/))
      album.id = `POP${lastID+1}`
      state.pop.push(album)
    },
    ADD_ALBUM_RAP: (state, album)=>{
      let lastID = parseInt(state.rap[state.rap.length-1].id.match(/\d+/))
      album.id = `RAP${lastID+1}`
      state.rap.push(album)
    },
    ADD_ALBUM_ROCK: (state, album)=>{
      let lastID = parseInt(state.rock[state.rock.length-1].id.match(/\d+/))
      album.id = `ROCK${lastID+1}`
